...
| **5 Express score** | After file pick: type **5**. Score `--output-mode slim` (no profile / password / Full-Slim pick). One Excel workbook: **POI_Scores** + **Summary** (opens on **POI_Scores**; column width 12). Dates `service_date` / `last_worked_date` display as `mm/dd/yyyy`. Generated CSVs are **removed** after a successful Excel write. |
