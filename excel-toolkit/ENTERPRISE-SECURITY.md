...
| **Identity** | Does not read credentials, tokens, or browser stores. Workbook open passwords are accepted only via interactive SecureString prompt (import open; optional menu export protect) or optional CLI `-Password` for automation. |
