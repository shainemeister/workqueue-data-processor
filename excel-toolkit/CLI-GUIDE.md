...
**Toolkit version:** 1.6.0 (see `version` command / `Get-ExcelToolkitVersion`)
...
| Interactive user | `Start-ExcelMenu.cmd` — **Process my data** lists `import\` CSV/Excel; multi-select accepts `1`, `1,2`, `1-3`, `1,3-5,8`, or a full path; optional workbook password on Excel export |
...
Without `-Json`, stdout is the bare version string (for example `1.6.0`).
...
CLI and module version are aligned at **1.6.0** via `Get-ExcelToolkitVersion` / `version` command. Bump when shipping breaking CLI contract changes (verbs, exit codes, JSON field names).
